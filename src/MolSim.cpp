...
VelDenProfile* VelDenPr = NULL;		//!< Class for profiling density and velocity.
...
PapiProfile* papiProfiler = NULL;	//!< Object for collecting PAPI profiling results.
...
int main(int argc, char* argsv[]) {
	...
	while (current_time < end_time) {
		...
		// update velocity and density profile
		if( VelDenPr != NULL ) {
			VelDenPr->analyse(iteration);
		}
	...
	//Collect PAPI profiling
	if (papiProfiler != NULL) {
		papiProfiler->stop();
	}
	...
	for (int i = 1; i < argc; i++) {
		if (argsv[i][0] == '-') {
			...
			if (strcmp(option, "help") == 0) {
				...
				//Initialize PAPI
				papiProfiler = new PapiProfile(value);
				papiProfiler->start();
			}
		} else {
