...
Particle::Particle(const Particle& other) :
		x(other.x), v(other.v), f(other.f), old_f(other.old_f), m(other.m), epsilon(
				other.epsilon), sigma(other.sigma), type(other.type), virt(other.virt) {
	for(int i=0; i<8; i++)
...
Particle::Particle(utils::Vector<double, 3> x_arg,
		...
		double epsilon_arg, double sigma_arg, bool virt_arg)
	:
		x(x_arg), v(v_arg), m(m_arg), epsilon(epsilon_arg), sigma(sigma_arg),
		type(type_arg), f(0.0), old_f(0.0), virt(virt_arg)
{
	for(int i=0; i<8; i++) neighbors[i]=NULL;
	LOG4CXX_TRACE(Particlelogger, "Particle generated!");
}
...
Particle::Particle(utils::Vector<double, 3> x_arg, utils::Vector<double, 3> v_arg,
		...
		double m_arg, double epsilon_arg, double sigma_arg, int type_arg, bool virt_arg)
	:
		x(x_arg), v(v_arg), f(f_arg), old_f(old_f_arg), m(m_arg),
		epsilon(epsilon_arg), sigma(sigma_arg), type(type_arg), virt(virt_arg)
{
	for(int i=0; i<8; i++) neighbors[i]=NULL;
	LOG4CXX_TRACE(Particlelogger, "Particle generated!");
}
 ...
 */
Particle* Particle::getNeighbor(int side) {
 ...
 *
 * @param side Number indicating on which side the neighbor is.
 * @param p Particle to define as a neighbor.
 */
